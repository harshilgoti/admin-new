          ...
          {({ ref }) => (
            <div ref={ref}>
              ...
              >
                ...
                <ListItemText className="list-item-text" primary={item.translate ? t(item.translate) : item.title} classes={{ primary: "text-14" }} />
                      ...
                      {item.children && (
                        <ul className={clsx(classes.children, "popper-navigation-list", dense && "dense", "px-0")}>
                          {item.children.map(_item => (
                            <FuseNavItem key={_item.id} type={`horizontal-${_item.type}`} item={_item} nestedLevel={nestedLevel + 1} dense={dense} />
                          ))}
                        </ul>
                      )}
                    </Paper>
