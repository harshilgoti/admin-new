...
function UserNavbarHeader(props) {
  ...
  return (
    ...
    >
      ...
      {user ? (
        user.image_url ? (
          <Avatar className={clsx(classes.avatar, "avatar")} alt="user photo" src={user.image_url} />
        ) : (
          <Avatar className={clsx(classes.avatar, "avatar")}>
            <div style={{ color: "white", fontSize: "24px" }}>{user.first_name && user.first_name[0].toUpperCase()}</div>
          </Avatar>
        )
      ) : (
        ""
